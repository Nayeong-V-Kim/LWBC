...
if not config.local:
    wandb.init(
        config=config,
        project='LWBC_celebA_Blond_Hair',
        anonymous='allow',
    )

# if config.dataset == 'celebA_Blond_Hair' or config.dataset =='celebA_Heavy_Makeup':
EVALSPLIT = num_correct_all
split_num = 4
val_metric = 'val_conflicting'
test_metric = 'test_conflicting'
test_worst_metric = 'test_worst_group'
...
for epoch in range(config.epochs):
    ...
    if epoch+1 > -1:
        ...
        val_conflicting =(valid_avg_accuracy_split[-1][0]+valid_avg_accuracy_split[-1][3])/2
        print("Epoch {} Train loss: {:.4f} target_loss: {:.4f} train_acc {:4f} val_conflicting {:4f}".format(epoch+1, losses.avg, target_losses.avg, train_avg_accuracy[-1], val_conflicting))
        
        # if not config.local:
        metrics['train_acc'] = train_avg_accuracy[-1]
        metrics['val_conflicting'] = (valid_avg_accuracy_split[-1][0]+valid_avg_accuracy_split[-1][3])/2
        ...
        for k in range(num_classifier):
            ...
            metrics['val_conflicting_M{}'.format(k)]=(valid_avg_accuracy_split[k][0]+valid_avg_accuracy_split[k][3])/2
        
        if best_metric[val_metric] < metrics[val_metric]:
            best_metric = metrics.copy()
            best_metric['epoch'] = epoch+1
            best_metric[test_metric] = (avg_accuracy_split[-1][0]+avg_accuracy_split[-1][3])/2
            best_metric[test_worst_metric] = min(avg_accuracy_split[-1])
            best_metric['test_unbiased_acc'] = avg_accuracy_split[-1].mean()
            best_metric['test_guiding_acc'] = (avg_accuracy_split[-1][1]+avg_accuracy_split[-1][2])/2

        print("best epoch {} {} {:.4f} {} {:.4f} {} {:.4f} {} {:.4f} {} {:.4f}".format(best_metric['epoch'], val_metric, best_metric[val_metric], test_metric, best_metric[test_metric].item(), test_worst_metric, best_metric[test_worst_metric].item(), 'test_unbiased_acc', best_metric['test_unbiased_acc'].item(), 'test_guiding_acc', best_metric['test_guiding_acc'].item()))
